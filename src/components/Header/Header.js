...
import { SIGNOUT_USER_SUCCESS } from "../../store/constants/ActionTypes";
        ...
        <li className="nav-item">
          <Dropdown alignRight>
            ...
            <Dropdown.Menu>
              <Dropdown.Item
                eventKey="1"
                onSelect={(ev, obj) => this.handleDashborad()}
              >
                Profile
              </Dropdown.Item>
              <Dropdown.Divider />
              ...
              <Dropdown.Item
                eventKey="4"
                onSelect={(ev, obj) => this.handleLogout(ev)}
              >
                Suggest Meal
              </Dropdown.Item>
              <Dropdown.Divider />
        ...
        >
          ...
          <div className="header-panel w-100">
            <div className="header-left ">
              ...
              <div
                className=" form-inline navbar-first"
            ...
            <div className="header-right navbar-first">
              <ul className="navbar-nav flex-row">
                ...
                <li className="nav-item">
                  <Link to="/SuggestMeal" className="nav-link px-3">
                    Suggest Meal
                  </Link>
                </li>
                {login_on_desktop_navbar}
...
const mapStateToProps = ({ auth, commonData }) => {
  const { authUser, role, customer_id, isAuthenticated } = auth;
  const { status } = commonData;
  return { authUser, role, customer_id, status, isAuthenticated };
};

export default connect(mapStateToProps, (dispatch) => ({
  logout: () => dispatch({ type: SIGNOUT_USER_SUCCESS }),
}))(withRouter(Header));
