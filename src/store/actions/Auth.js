...
export const authUser =
  ({ setSubmitting, history, values, isSignup }) =>
  (dispatch) => {
    axios
      .post(`/auth/${isSignup ? "signup" : "login"}`, values)
      .then(({ data: { token, ...rest } }) => {
        setSubmitting(false);
        toast("Welcome, you have logged in successfully", { type: "success" });
        history.push("/");
        localStorage.setItem("accessToken", token);
        dispatch({ type: USER_DATA, payload: rest });
      })
      .catch((error) => {
        toast(
          error?.response
